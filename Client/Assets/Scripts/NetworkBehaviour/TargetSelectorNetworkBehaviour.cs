...
public class TargetSelectorNetworkBehaviour : NetworkBehaviour
{
    ...
    private void HandleSelectionInput()
    {
        ...
        if (mouse.leftButton.wasPressedThisFrame)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hit) && hit.transform.tag == "Target")
            {
                ...
                _targetCanvas.transform.Find("Target/HealthPoints").GetComponent<TextMeshProUGUI>().text = SelectedTargetTransform.GetComponent<HealthNetworkBehaviour>().Health.Value.ToString();
            }
        }
    }
    ...
    public void SpawnProjectileServerRpc(Vector3 position, Vector3 direction, ulong clientId, string token)
    {
        ...
        var spawnedFireball = fireball.GetComponent<FireballNetworkBehaviour>();
    ...
    {
        var fireball = NetworkManager.Singleton.SpawnManager.SpawnedObjects[objectId].GetComponent<FireballNetworkBehaviour>();
    ...
    private void FailedServerRpc(ulong objectId, ulong clientId)
    {
        ...
        var _fireball = netObj.GetComponent<FireballNetworkBehaviour>();
