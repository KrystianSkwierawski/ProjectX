...
public class FireballNetworkBehaviour : NetworkBehaviour
{
    ...
    private async UniTask OnHitTargetAsync()
    {
        if (!_hit)
        {
            _hit = await _target.GetComponent<HealthNetworkBehaviour>().DealDamageAsync(50f, _clientToken, OwnerClientId);
