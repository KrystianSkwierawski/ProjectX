...
- Inventory drag-and-drop is owner-optimistic and server/API-persisted. `InventoryUI` instantiates the serialized `InventoryDragPreview` prefab as a non-raycastable cursor-following icon/count preview and submits source/target indices through `MoveInventorySubscription` and a dedicated ServerRpc. Different item types swap, identical types merge their counts, and empty targets retain their exact index. `InventoryItemEnum.None`/count `0` entries are stable empty-slot placeholders; full removal clears a slot, while add and split operations prefer existing empty slots.
